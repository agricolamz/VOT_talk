...
```{r}
final_df %>% 
  filter(closdur > 0, VOT > 0, closdur < 310, position == "medial", affric == "plain") %>% 
...
final_df %>% 
  ...
  theme_bw()+
  theme(text = element_text(size=20), legend.position="bottom")+
  labs(x = "", y = "VOT (ms)", caption = paste("Based on",
                       length(unique(results$filename)),
                       "speakers"))+
  facet_grid(Gender~Language, margins = "Gender", scales = "free_x")
```
...
final_df %>% 
  ...
  theme_bw()+
  theme(text = element_text(size=20), legend.position="bottom")+
  labs(x = "CD (ms)", y = "VOT (ms)", caption = paste("Based on",
                       length(unique(results$filename)),
                       "speakers"))+
...
```{r}
final_df %>% 
  filter(closdur > 0, VOT > 0, closdur < 310, position == "medial", affric == "affricate") %>% 
...
```{r}
final_df %>% 
  filter(closdur > 0, VOT > 0, closdur < 310, postfric > 0, position == "medial", affric == "affricate") %>% 
  ggplot(aes(c_type, postfric, fill = aff_type))+
  geom_boxplot()+
  theme_bw()+
  theme(text = element_text(size=20), legend.position="bottom")+
  labs(x = "", y = "post-fricative part (ms)", caption = paste("Based on",
                       length(unique(results$filename)),
                       "speakers"))+
  facet_grid(Gender~Language, margins = "Gender", scales = "free_x")
```
...
final_df %>% 
  ...
  theme_bw()+
  theme(text = element_text(size=20), legend.position="bottom")+
  labs(x = "CD (ms)", y = "post-fricative part (ms)", caption = paste("Based on",
                       length(unique(results$filename)),
                       "speakers"))+
  facet_grid(Gender~Language, margins = "Gender")
```
...
--
  ...
  - agreement per each of the features that we annotated (**Cohen’s kappa**);
  - agreement for time segmentation that we annotated (standard deviation). --
