...
```{r}
final_df %>% 
  filter(position == "medial", affric == "affricate") %>%
  ggplot(aes(stop, closdur, fill = Language, label = stop_label))+
  geom_boxplot()+
  theme_bw()+
  theme(legend.position="bottom")+
  facet_grid(Gender~., scale = "free_x", margins = "Gender")+
  labs(y = "CD (ms)", x = "", caption = paste("Based on",
                       length(unique(results$filename)),
                       "speakers"))
```
...
final_df %>% 
  filter(position == "medial", affric == "affricate") %>%
  ggplot(aes(stop, VOT, fill = Language, label = stop_label))+
  geom_boxplot()+
  theme_bw()+
  theme(legend.position="bottom")+
  facet_grid(Gender~., scale = "free_x", margins = "Gender")+
  labs(y = "VOT (ms)", x = "", caption = paste("Based on",
                       length(unique(results$filename)),
                       "speakers"))
```
...
final_df %>% 
  filter(position == "medial", affric == "affricate") %>%
  ggplot(aes(closdur, VOT, fill = Language, label = stop_label))+
  geom_point(aes(color = Language), alpha = 0.2)+
  stat_ellipse()+
  theme_bw()+
  theme(legend.position="bottom")+
...
final_df %>% 
  filter(position == "medial", affric == "affricate") %>%
  ggplot(aes(stop, postfric, fill = Language, label = stop_label))+
  geom_boxplot()+
  theme_bw()+
  theme(legend.position="bottom")+
  facet_grid(Gender~., scale = "free_x", margins = "Gender")+
...
final_df %>% 
  filter(position == "medial", affric == "affricate") %>%
  ggplot(aes(closdur, postfric, fill = Language, label = stop_label))+
  geom_point(aes(color = Language), alpha = 0.2)+
  stat_ellipse()+
  theme_bw()+
  theme(legend.position="bottom")+
