...
map.feature(df_u$Language,
            ...
            color = df_u$color,
            label.emphasize = list(1:13, "#BD2D2D"))
```
---
# Speakers by village
```{r}
...
```{r}
final_df %>% 
  filter(closdur > 0, closdur < 0.31, folVowDur> 0, position == "medial", affric == "plain") %>% 
...
```{r}
final_df %>% 
  filter(closdur > 0, closdur < 0.31, folVowDur> 0, position == "medial", affric == "affricate") %>% 
